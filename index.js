...
function temperatureInF(temp,unit) {
  return (unit === 'F') ? temp+'F' : temperatureCtoF(temp) + 'F'
}
...
function makePersonObject(id,name,email) {
  return  this.personObj = {"id":id,"name":name,"email":email }
}
...
function getName(arg) {
  return `Hello,my name is ${arg.name}`;
}
...
function appleIndex(fruits) {
   for (let i = 0; i< fruits.length; i++){if (fruits[i]==='apple') return i;}
  }
